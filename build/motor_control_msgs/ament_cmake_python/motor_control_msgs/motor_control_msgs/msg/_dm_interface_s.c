...
bool motor_control_msgs__msg__dm_interface__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("motor_control_msgs.msg._dm_interface.DmInterface", full_classname_dest, 48) == 0);
  }
  motor_control_msgs__msg__DmInterface * ros_message = _ros_message;
  {  // header
  ...
  {  // dq
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->dq.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
{
  /* NOTE(esteve): Call constructor of DmInterface */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("motor_control_msgs.msg._dm_interface");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "DmInterface");
  ...
  {  // dq
    ...
    if (ros_message->dq.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->dq.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->dq.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
